      ...
      <div className="bg-white rounded-lg shadow-sm border p-4 sm:p-6">
       ...
       {/* 棒グラフ（棒の中に平均速度を表示） */}
        ...
        {!isRangeMode ? (
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={singleDayStatusData} margin={{ top: 20, right: 30, left: 20, bottom: 60 }}>
                <CartesianGrid strokeDasharray="3 3" />
                ...
                <Bar dataKey="height" radius={[4, 4, 0, 0]}>
                  {singleDayStatusData.map((entry, index) => (
              ...
              return (
                <div key={dayData.date} className="border rounded-lg p-4">
                  <h4 className="text-md font-medium text-gray-800 mb-3">
                    {dayData.date} ({new Date(dayData.date).toLocaleDateString('ja-JP', { weekday: 'short' })})
                  </h4>
                  <div className="h-48">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={dayStatusData} margin={{ top: 10, right: 20, left: 10, bottom: 40 }}>
                        <CartesianGrid strokeDasharray="3 3" />
                        <XAxis 
                          dataKey="hour" 
                          tick={{ fontSize: 11 }} 
                        ...
                        <Tooltip
                          ...
                          labelFormatter={(label) => `${dayData.date} ${label}`}
                        />
                        <Bar dataKey="height" radius={[3, 3, 0, 0]}>
                          {dayStatusData.map((entry, index) => (
                            <Cell key={`cell-${dayIndex}-${index}`} fill={entry.color} />
                          ))}
                          <LabelList
                            dataKey="avgSpeed"
                            position="center"
